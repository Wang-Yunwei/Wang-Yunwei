...
```
- Eureka Server配置项 (server与client关联配置)
```text
...
```
- Eureka Server配置项(server 自定义实现的配置)
```text
#json的转换的实现类名
...
```
- Eureka Server配置项 (server node 与 node 之间关联的配置)
```text
...
```
- Eureka Server配置项 (server 与 remote 关联的配置)
```text
#过期数据，是否也提供给远程region
...
```
- Eureka Server配置项 (server 与 ASG/AWS/EIP/route52 之间关联的配置)
```text
#缓存ASG信息的过期时间。
